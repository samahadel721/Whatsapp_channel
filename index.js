...
const TARGET_CHANNEL = clean(process.env.TARGET_CHANNEL);
const MONITORED_CHANNELS = parseJidList(process.env.MONITORED_CHANNELS);
...
async function forwardMessage(sock, msg, from) {
    ...
    if (text) {
        await sock.sendMessage(TARGET_CHANNEL, { text });
    ...
    if (mediaInfo) {
        ...
        await sock.sendMessage(TARGET_CHANNEL, payload);
    ...
    if (contentType === 'locationMessage') {
        ...
        await sock.sendMessage(TARGET_CHANNEL, {
            location: {
...
function printConfiguration() {
    ...
    if (!TARGET_CHANNEL || MONITORED_CHANNELS.size === 0) {
    ...
    console.log(`🎯 القناة المستهدفة: ${TARGET_CHANNEL}`);
    console.log(`👀 عدد القنوات المراقبة: ${MONITORED_CHANNELS.size}`);
}
...
async function startBot() {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
            ...
            if (!DISCOVERY_MODE && TARGET_CHANNEL && MONITORED_CHANNELS.size > 0) {
...
async function handleMessage(sock, msg) {
    ...
    if (!MONITORED_CHANNELS.has(from) || (TARGET_CHANNEL && from === TARGET_CHANNEL)) return;
